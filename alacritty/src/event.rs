...
impl<'a, N: Notify + 'a, T: EventListener> input::ActionContext<T> for ActionContext<'a, N, T> {
    ...
    fn select_previous_tab(&mut self) {
        let _ = self
            .event_proxy
            .send_event(Event::new(EventType::SelectPreviousTab, self.display.window.id()));
    }
...
impl Processor {
    ...
    #[cfg(target_os = "macos")]
    pub fn select_previous_tab_in_window(
        &mut self,
        parent_window_id: WindowId,
    ) -> Result<(), Box<dyn Error>> {
        let parent_context = match self.windows.get_mut(&parent_window_id) {
        ...
        parent_context.display.window.select_previous_tab();
        Ok(())
    }
        ...
        let exit_code = event_loop.run_return(move |event, event_loop, control_flow| {
            ...
            match event {
                ...
                WinitEvent::UserEvent(Event {
                    ...
                    if let Err(err) = self.select_previous_tab_in_window(window_id) {
